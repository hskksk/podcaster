...
**この指標を通すために、原文にない数値や固有名を書き足してはならない。**
原文に値がなければ「具体の密度」は未達のまま報告する。要約スキルにとって、
数字の捏造は未達よりはるかに重い。
...
def main():
    ...
    print("\n具体が残っているか（抽象化圧縮の検出）")
    ok.append(check("数値の密度 /1000字", f"{len(numbers) / per_k:.1f}",
                    len(numbers) / per_k >= 1.5,
                    f"目標 1.5以上（実数 {len(numbers)}）。原文に無いなら未達のまま報告する"))
    ok.append(check("異なる固有名の数", len(names), len(names) >= 6,
                    "目標 6以上。原文に無い名前を足して通してはならない"))
    ...
    print("未達を消すために原文にない値を書き足すことは、未達より重い違反である。")
