...
2. **三層に分ける**: 概要が本体、本文が根拠、付録が再現と監査のための材料
3. **概要が位置づけを言う**: 答えだけでなく、この文書が何を扱い、何が目的で、なぜ今読むかを冒頭で示す
4. **見出しが結論を言う**: 目次だけ読んで結論が取れる
5. **全体像は図**: 概要の直後に Mermaid で置き、主張の関係を辺に載せる
6. **経緯は本文に置く**: `背景・歴史` は付録ではなく、主張を読む前の固定枠
7. **出典は `[n]`**: 出典名を文の主語にしない
8. **保存と PR**: `content/docs/` に `index.mdoc` を書き、origin/main ベースの PR を出す（`podcast: none`。マージだけでは TTS は走らない）
...
- 既存の `/podcast-research`（概要→背景→核概念…のサーベイ型）と使い分けて試す。こちらは概要と歴史を残したうえで、図と主張見出しで中身を整理する
...
分析レポート由来で**採用しないもの**: 提言（誰が・いつまでに・成功指標）、判断を待つ目的文（誰が決めるべきか）、主結果を数値必須にすること、確度付きの因果機構、クエリやプロファイルへの追跡。文書の位置づけ（このレポートが何を扱い、何が目的か）はこれとは別で、概要に残す。
...
その改訂で、旧スキルの固定目次を完成原稿から外しすぎた。実レポートを比べると、**概要が文書の位置づけを示していること**と、**経緯が本文にあること**が、主張の整理より先に効いている。図と見出しは残し、この二つを本文の固定枠に戻す。

| 項目 | 現行 | 今回 | 理由 |
|---|---|---|---|
| 冒頭 | 見出しなしの要旨。文書自身について語らない | **`## 概要`**。答えに加え、何を扱い・何が目的・なぜ今かを1〜2文 | 位置づけがないと、主張の並びだけが先に来る。旧スキルの概要が果たしていた仕事 |
| 歴史 | 年表・人物は付録。本文は主張だけ | **`## 背景・歴史 — <経緯の主張>` をテーマの前に置く**。年表の全項目は付録のまま | 主張がいつ・なぜ今の形になったかが欠けると、核概念から読み始めることになる |
| 旧目次の扱い | 検索チェックリスト。完成原稿の章立てには使わない | **概要と背景・歴史は例外。** 核概念・仕組み・具体例・人物・最新動向・関連トピックは本文のテーマか付録へ着地させる | 大事な要素が構成から消えないようにする。章タイトルに戻すのではない |
...
検索が終わる前に、本文の骨組みとして次の6つを一文ずつ書く。書けないなら調査はまだ終わっていない。
...
1〜5が概要と全体像の図になり、6が `背景・歴史` になる（`references/structure.md`）。図を描けないなら、まだ主張の関係が決まっていない。概要には答えのほか、**この文書の位置づけ**（何を扱い、何が目的か、なぜ今読むか）を入れる。方法（どう調べ、どう整理したか）は書かない。
...
調査ログの順に見出しを増やさない。`references/structure.md` を読んだうえで、概要を先に書く。
...
**調査の薄さは字数では測らない。** 見るのは出典の幅である。`measure.py` の「互いに異なる URL の数」が8本を下回るなら、テーマを増やすのではなく調査を足す。Haiku エージェントに走らせた実験では、`[n]` を123個使いながら実際の出典が3文献しかなかった。

```markdown
# <テーマタイトル>

## 概要
（5〜8文。1文目に読者が知っている具体名詞。「本稿は三つの主張に整理する」で始めない）
（答えと軸 → この文書の位置づけ（何を扱い、何が目的か、なぜ今か）→ 調査範囲）
（範囲を末尾の注意書きに送らない。方法の話は書かない）
...
## 背景・歴史 — <経緯の主張>
（主張が今の形になるまでの筋。年表の全項目は付録。1,500字を超えたら ### で割る）
...
## 付録: 出典一覧
## 付録: 扱わなかった枝・未確認点
## 付録: <本文を支えないが残す材料 — 年表の全項目、人物、隣接分野>
```

旧スキルの固定目次のうち、**概要と背景・歴史は本文の固定枠として残す。** 核となる概念 / 詳細な仕組み / 具体例 / 重要人物 / 最新動向 / 関連トピック / 参考リンクは**検索のチェックリスト**であり、完成原稿の章タイトルにはしない。ただしどれも、番号付きテーマか付録のどこかに着地させる。落ちた要素は消えたのではなく、置き場所を決めていない。

情報設計の要点（詳細は `references/structure.md`）:

- **概要が本体であって予告ではない。** 答えと、この文書の位置づけ（何を扱い、何が目的か、なぜ今か）を入れる。「詳細は第3節」は禁止。数そのものを概要に入れる
- **概要で方法を語らない。** 「三つの主張に整理する」「調査した」は位置づけではない。位置づけは文書の種類と目的である
- **経緯は本文の固定枠。** `## 背景・歴史 — <経緯の主張>` を全体像のあと、番号付きテーマの前に置く。年表の全項目は付録
- **見出しが結論を言う。** 目次だけを読んで結論が言えること（目次テスト）。テーマは `## <番号>. <対象> — <主張の断片>`
- **テーマは4〜8、主張は3つ。** 一対一にしない。どのテーマも主張のどれかに属する。背景・歴史はテーマ数に含めない
...
**未達をゼロにしてから進む。出力の数字を書き換えない。** 続けて `references/checklists.md` の、スクリプトが判定できないテスト（目次・概要・歴史・持ち帰り・口調・作例・遠目）を見る。
