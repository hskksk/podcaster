...
- **節の主張は `##` の直下に置く。** 第一文が主張、その段落までが最初の `###` の前。小節は根拠の側にだけ作る。見取り図への寄与を述べる段落は、最後の小節の末尾に置く（Markdown では小節を閉じて `##` の水準に戻れない）。
...
上限は L1 と L2 の散文にかける。L3 の出典一覧や年表のように、行が並ぶだけの付録は長くてよい。
