...
`podcast-research` の調査手順・保存先・PR フローはそのまま使い、レポートの**情報設計**だけを差し替えた実験版です。しばらくは既存スキルと併用する。分析レポート用の規則（提言、判断待ちの lede、クエリ追跡、Finding→Action の語彙）は入れない。論の骨格は図で置くが、ノードはリサーチ用の語彙にする。

1. **広く調べ、狭く書く**: 8〜12 のサブトピックを検索するが、本文に残す主張は 3 つ（±1）
2. **三層開示**: L1 要旨がレポート本体。L2 が根拠。L3 が再現・監査用
3. **論の骨格は図**: L1 の直後に Mermaid の骨格図を置き、主張の関係を辺に載せる
4. **保存と PR**: `content/docs/` に `index.mdoc` を書き、origin/main ベースの PR を出す（`podcast: none`。マージだけでは TTS は走らない）
...
| 参照 | 役割 | いつ |
|------|------|------|
| `references/structure.md` | 三層開示、lede の 6 スロット、骨格図、節の契約、見出し、長さの上限、Before/After | **本文を書き始める前に必須** |
...
分析レポート由来で**採用しないもの**: 提言（誰が・いつまでに・成功指標）、判断を待つ Purpose、主結果を数値必須にする Result、確度付きの因果機構、クエリ／プロファイルへの追跡。

論の骨格図は**採用する**。ただしノードの語彙は Finding→Action ではなく、次のステップ 1 で決める 5 つ（問い・見取り図・枠組み・三つの主張・範囲外）＋射程に入れ替える。
...
この 5 つが、そのまま本文冒頭の**骨格図**のノードになる（`references/structure.md` の「骨格ブロック」）。図を描けないなら、まだ主張の関係が決まっていない。
...
## 骨格
<!-- 論の骨格は図で置く。{% diagram type="mermaid" %} の中をフェンスで包む -->
<!-- ノード: 問い / 枠組み / 主張1-3 / 見取り図 / 範囲外 / 射程。辺のラベルが関係を運ぶ -->
<!-- 図の下に、関係（連鎖・並列・対比）を名指す一文を添える -->

## <主張 1 が扱う対象の名詞句>
（第一文が主張。続く段落が根拠。節末が見取り図への寄与）
（1,500 字を超えたら ### で 2〜4 の小節に割る）
...
- **推論は段落、一覧は目録。** 節の本体が箇条書きだけ、は禁止。逆に、同じ列を持つものが四つ以上並ぶ一覧を散文に流し込むのも禁止。資料の列挙は表にする
- **一息を数える。** 一文 100 字、段落 5 文／250 字、小節 1,500 字が上限。節が 1,500 字を超えたら `###` を入れる。主張が三つでも、節が三つの壁になったら失敗
- **出典の帰属は段落に一度。** 文ごとに「〜は述べる」「〜は書く」と主語が入れ替わる段落は、論ではなく書誌になる
