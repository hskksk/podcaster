# markdoc/next.js: Markdoc plugin for Next.js
...
2.  Open `next.config.js` and add the following code:
    ...
    Turbopack currently requires every schema entry file referenced by `schemaPath` to exist, even if you are not customizing them yet. Create `config.js`, `nodes.js`, `tags.js`, and `functions.js` in that directory (exporting empty objects is fine) so the loader can resolve them during the build.
...
This project uses the [MIT license](https://github.com/markdoc/next.js/blob/main/LICENSE).